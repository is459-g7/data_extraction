...
for airport_code, info in airport_flight_data.items():
    sorted_dates = sort_dates(info['missing_dates'])
...
with open('missing_weather_data_sorted.json', 'w') as f:
    json.dump(airport_flight_data, f, indent=4)

print("Dates have been sorted and saved in 'missing _weather_data_sorted.json'.")
